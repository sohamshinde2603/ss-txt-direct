...
#........................................................................................................................................................................................
    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            original_url = url
            keys_string = ""
            add_web_headers = False
            acecwply_flag = False
            api_data_keys = [] # Store raw keys for mp4decrypt
#........................................................................................................................................................................................
...
#........................................................................................................................................................................................
            if "visionias" in original_url:
                async with ClientSession() as session:
                    async with session.get(original_url, headers={'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9', 'Accept-Language': 'en-US,en;q=0.9', 'Cache-Control': 'no-cache', 'Connection': 'keep-alive', 'Pragma': 'no-cache', 'Referer': 'http://www.visionias.in/', 'Sec-Fetch-Dest': 'iframe', 'Sec-Fetch-Mode': 'navigate', 'Sec-Fetch-Site': 'cross-site', 'Upgrade-Insecure-Requests': '1', 'User-Agent': 'Mozilla/5.0 (Linux; Android 12; RMX2121) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/107.0.0.0 Mobile Safari/537.36', 'sec-ch-ua': '"Chromium";v="107", "Not=A?Brand";v="24"', 'sec-ch-ua-mobile': '?1', 'sec-ch-ua-platform': '"Android"',}) as resp:
            ...
            if "classplusapp" in original_url:
               ...
               # "https://media-cdn.classplusapp.com/drm/63ef86e8eea5649b4007f10e/playlist.m3u8"
                api_call = f"https://itsgolu-cp-api.vercel.app/itsgolu?url={urllib.parse.quote(pre_api_url)}@ITSGOLU_OFFICIAL&user_id=8422547853"
                success = False
                try:
            ...
            # --------------------- FIXED BLOCK ---------------------
            # yt-dlp *never* takes the --key argument directly.
            base_cmd = 'yt-dlp ' 
            # -------------------------------------------------------

            if acecwply_flag:
                cmd = f'{base_cmd}-o "{name}.%(ext)s" -f "bestvideo[height<={raw_text2}]+bestaudio" --hls-prefer-ffmpeg --no-keep-video --remux-video mkv --no-warning "{url}"'
            elif "jw-prod" in url:
                cmd = f'{base_cmd}-o "{name}.mp4" "{url}"'
            elif add_web_headers:
                cmd = f'{base_cmd}--add-header "referer:https://web.classplusapp.com/" --add-header "x-cdn-tag:empty" -f "{ytf}" "{url}" -o "{name}.mp4"'
            elif "youtube.com" in url or "youtu.be" in url:
                cmd = f'{base_cmd}--cookies youtube_cookies.txt -f "{ytf}" "{url}" -o "{name}".mp4'
            
            # --------------------- FIXED BLOCK ---------------------
            # Remove the `elif keys_string:` block from here, as it was incorrect.
            # The standard `else` will now handle non-DRM, non-YouTube links.
            # The DRM logic is moved to the download section below.
            # -------------------------------------------------------
            else: 
                cmd = f'{base_cmd}-f "{ytf}" "{url}" -o "{name}.mp4"'
...
#........................................................................................................................................................................................           
                if "drive" in url:
                    ...
                    if "cwmediabkt99" in url:
                        ...
                        for attempt in range(max_retries):
                            try:
                                ...
                                if response.status_code == 200:
                                    ...
                                    # --- Start Change for FloodWait and Delay ---
                                    try:
                                        copy = await bot.send_document(chat_id=channel_id, document=f'{namef}.pdf', caption=cc1)
                                        await asyncio.sleep(1) # Added 1-second delay after successful upload
                                    except FloodWait as e:
                                        await m.reply_text(f"FloodWait: Waiting for {e.value} seconds before retrying PDF upload...")
                                        await asyncio.sleep(e.value) # Wait for the required time
                                        copy = await bot.send_document(chat_id=channel_id, document=f'{namef}.pdf', caption=cc1) # Retry upload
                                        await asyncio.sleep(1) # Added 1-second delay after successful upload retry
                                    # --- End Change ---
                        ...
                        try:
                            ...
                            await m.reply_text(str(e))
                            time.sleep(e.x) # Note: e.x is generally used in Pyrogram FloodWait, but `e.value` is often preferred for clarity. I'm keeping `time.sleep(e.x)` here for consistency with your existing non-PDF error handling, but the inner blocks use `asyncio.sleep(e.value)` which is more appropriate for the async context.
                            continue   
                ...
                # --------------------- FIXED BLOCK ---------------------
                # This new block handles DRM content using mp4decrypt
                elif keys_string:
                    prog = await bot.send_message(channel_id, Show, disable_web_page_preview=True)
                    prog1 = await m.reply_text(Show1, disable_web_page_preview=True)
                    
                    v_name_enc = f"{name}.enc_v.mp4"
                    a_name_enc = f"{name}.enc_a.m4a"
                    v_name_dec = f"{name}.dec_v.mp4"
                    a_name_dec = f"{name}.dec_a.m4a"
                    filename = f"{name}.mkv" # Final output name

                    try:
                        # 1. Download encrypted video
                        #await prog.edit(f"<blockquote>{Show}\n\n<b>Phase 1/4: Downloading encrypted video...</b></blockquote>")
                        video_cmd = f'yt-dlp -f "bestvideo[height<={raw_text2}]" --allow-unplayable-formats "{url}" -o "{v_name_enc}"'
                        os.system(video_cmd)

                        # 2. Download encrypted audio
                        #await prog.edit(f"<blockquote>{Show}\n\n<b>Phase 2/4: Downloading encrypted audio...</b></blockquote>")
                        audio_cmd = f'yt-dlp -f "bestaudio" --allow-unplayable-formats "{url}" -o "{a_name_enc}"'
                        os.system(audio_cmd)
                        
                        if not os.path.exists(v_name_enc) or not os.path.exists(a_name_enc):
                            raise Exception("Failed to download encrypted media files. Check yt-dlp logs.")

                        # 3. Decrypt
                        #await prog.edit(f"<blockquote>{Show}\n\n<b>Phase 3/4: Decrypting media...</b></blockquote>")
                        
                        decrypt_v_cmd = f'mp4decrypt {keys_string} "{v_name_enc}" "{v_name_dec}"'
                        os.system(decrypt_v_cmd)
                        
                        decrypt_a_cmd = f'mp4decrypt {keys_string} "{a_name_enc}" "{a_name_dec}"'
                        os.system(decrypt_a_cmd)
                        
                        if not os.path.exists(v_name_dec) or not os.path.exists(a_name_dec):
                            raise Exception("Failed to decrypt media files. Check mp4decrypt logs and ensure keys are correct.")

                        # 4. Merge
                        #await prog.edit(f"<blockquote>{Show}\n\n<b>Phase 4/4: Merging decrypted files...</b></blockquote>")
                        merge_cmd = f'ffmpeg -i "{v_name_dec}" -i "{a_name_dec}" -c copy "{filename}"'
                        os.system(merge_cmd)
                        
                        if not os.path.exists(filename):
                            raise Exception("Failed to merge decrypted files. Check ffmpeg logs.")
                        ...
                        for f in [v_name_enc, a_name_enc, v_name_dec, a_name_dec]:
                            if os.path.exists(f):
                                os.remove(f)
                # --------------------- END FIXED BLOCK -----------------
                    ...
                    prog = await bot.send_message(channel_id, Show, disable_web_page_preview=True)
                    prog1 = await m.reply_text(Show1, disable_web_page_preview=True)
                    # The 'cmd' variable is now correctly generated (without keys) from the logic block above
